...
# png setjmp nonsense
Patch5: png_setjmp_20141101.patch
# fixes for fortran dynamic memory, c standard header changes
# fixes for tk/tcl stuff
# fixes to kill some warnings

Patch6: 20241120_CELfixes.patch
...
# the demo programs only link with libname.so, not libname.so.version
# so build without the version, then mover and make links

./makemake . linux g77_gcc
# Parallel make not supported
%{__make} FC="gfortran" CC="%{__cc}" CFLAGS="%{optflags}" FFLAGS="%{optflags}" SHARED_LIB=libpgplot.so
%{__mv} lib%{name}.so lib%{name}.so.%{version}
%{__ln_s} lib%{name}.so.%{version} lib%{name}.so
# Creating dynamic library for C
%{__make}  %{?_smp_mflags} \
   FC=gfortran CC="%{__cc}" CFLAGS="%{optflags}" FFLAGS="%{optflags}" cpg
%{__ar} x libcpgplot.a
...
%changelog
* Wed Nov 20 2024 Chuck Lane <lane@dchooz.org> - 5.2.2-58
- PNG had a 'setjmp' problem, fixed
- fix fortran dynamic memory problems that was killing GIF/PPM/PNG
- various fixes to deal with warnings, Tk updates
- add code in .spec to fetch pgplot source if not already present,
-    and compare checksums to make sure it's the right one.
